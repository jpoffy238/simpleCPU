...
public class Test_LDA_ZP {
	...
	public void Test_exec() {
		int i = 0x1000;
		try {
			mem.write(i++, OpCodes.LDA_ZP.code());
			mem.write(i++, (byte) (0x20));
			mem.write(i++, OpCodes.HLT.code());
			mem.write(0x001f,(byte)0x01);
			mem.write(0x0020, (byte) 0x55);
			mem.write(0x0021,(byte)0xff);
		} catch (illegalAddressException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (DeviceUnavailable e) {
		...
		try {
			assert (result == mem.read(0x20));
			assert(result != mem.read(0x21));
			assert(result != mem.read(0x1f));
		} catch (illegalAddressException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (DeviceUnavailable e) {
